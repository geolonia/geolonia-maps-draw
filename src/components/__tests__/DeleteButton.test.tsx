...
describe('DeleteButton', () => {
  it('renders correctly with default title', () => {
    ...
    const btn = screen.getByTitle('選択した図形を削除')
  ...
  it('calls onClick when clicked', () => {
    ...
    fireEvent.click(screen.getByTitle('選択した図形を削除'))
    expect(onClick).toHaveBeenCalledTimes(1)
  })

  it('is disabled when disabled prop is true', () => {
    render(<DeleteButton disabled={true} onClick={vi.fn()} />)
    const btn = screen.getByTitle('選択した図形を削除') as HTMLButtonElement
  ...
  it('applies custom title', () => {
    ...
    const btn = screen.getByTitle('Custom Delete')
    expect(btn).not.toBeNull()
  })
